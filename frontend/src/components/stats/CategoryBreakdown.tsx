  ...
  if (categories.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-warm-200/60 shadow-sm p-4 sm:p-5">
        <h3 className="text-base font-semibold text-warm-700 mb-4">카테고리별 지출</h3>
        <p className="text-sm text-warm-400 text-center py-4">데이터가 없습니다</p>
      </div>
    )
  }
          ...
          return (
            <div key={cat.category} className="space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-warm-700">{cat.category}</span>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-warm-900">{formatAmount(cat.amount)}</span>
              ...
              <div className="text-xs text-warm-400 text-right">{cat.percentage}%</div>
            </div>
          )
        })}
      </div>
