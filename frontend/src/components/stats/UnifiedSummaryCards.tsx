...
function ChangeIndicator({ current, previous }: { current: number; previous: number }) {
  ...
  const color = pct >= 0 ? 'text-leaf-600' : 'text-red-500'
  return (
...
export default function UnifiedSummaryCards({
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
      ...
      {netWorth != null && (
        <div className="col-span-2 lg:col-span-4 bg-gradient-to-br from-warm-50 to-warm-100 border border-warm-200/60 rounded-2xl shadow-sm p-4 sm:p-5 text-center">
          <p className="text-sm text-warm-500 mb-0.5">순자산</p>
          <p className="text-xl sm:text-2xl font-bold text-warm-900">{formatLargeAmount(netWorth)}</p>
      ...
      <div className="bg-gradient-to-br from-leaf-50 to-leaf-100 border border-leaf-200/60 rounded-2xl shadow-sm p-4 sm:p-5">
        ...
        <p className="text-xl sm:text-2xl font-bold tracking-tight text-warm-900 mt-1">
