...
- **현재 상태**: FE 구현 대기 (현재는 가구별 뷰만 제공)

### D4: 공유 지출 수정 권한
- **결정**: 본인이 입력한 지출만 수정/삭제 가능 (owner/admin도 타인 지출 수정 불가)
- **이유**: 데이터 무결성 보장, 분쟁 방지. 커플이라도 상대방 기록을 직접 수정하는 것은 불편할 수 있음
- **변경 가능**: 향후 "admin이 모든 지출 수정 가능" 옵션 추가 고려

### D5: 가구 삭제 시 데이터 보존 정책
- **결정**: 가구 소프트 삭제 시 관련 지출/카테고리/예산의 household_id를 SET NULL로 처리
