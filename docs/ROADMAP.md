...
| 기능 | 상태 | 완료 조건 |
|------|------|----------|
| LLM 자연어 파싱 구현 | 예정 | "점심 8000원" -> 정확한 JSON 파싱 |
| 프롬프트 엔지니어링 | 예정 | 파싱 정확도 > 80% |
| Alembic 마이그레이션 초기화 | 예정 | `alembic upgrade head` 동작 |
| 프론트엔드 자연어 입력 UI | 예정 | 채팅 인터페이스 완성 |
| 파싱 결과 확인/수정 UI | 예정 | 사용자가 결과 수정 가능 |
