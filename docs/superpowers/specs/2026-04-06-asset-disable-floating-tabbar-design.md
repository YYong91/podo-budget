...
**배치 규칙:**
- 아일랜드 + 입력 버튼을 합한 전체가 **화면 하단 중앙 정렬**
- 아일랜드와 입력 버튼 사이 간격: `gap-3` (12px)
- 하단 여백: `bottom: env(safe-area-inset-bottom) + 12px`
- 두 요소를 감싸는 flex 컨테이너로 중앙 정렬
...
- 구현: 하나의 컨테이너 안에 탭바/입력창 두 레이어를 두고 `opacity` + `pointer-events` 전환
- 아일랜드와 입력 버튼이 합쳐지는 효과: 컨테이너 `width` transition으로 전체 너비 확장
