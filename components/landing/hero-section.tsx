...
const CHAT_SEQUENCE = [
  ...
  },
  {
    id: 4,
    type: "ai" as const,
    text: "기록됐어요! 오늘 식비는 총 18,500원이에요 🍇",
    delay: 2500,
  },
]
    ...
    <div className="relative h-[540px] w-[268px] overflow-hidden rounded-[3rem] border-[7px] border-stone-900/10 bg-white shadow-2xl md:h-[600px] md:w-[290px]">
      ...
      <div className="flex items-center justify-between bg-purple-600 px-5 pb-2 pt-3 text-[10px] text-white/90">
        ...
        <div className="flex gap-1">
          ...
          <svg viewBox="0 0 24 24" className="h-3 w-3 fill-current"><path d="M15.67 4H14V2h-4v2H8.33C7.6 4 7 4.6 7 5.33v15.33C7 21.4 7.6 22 8.33 22h7.33c.74 0 1.34-.6 1.34-1.33V5.33C17 4.6 16.4 4 15.67 4z"/></svg>
        </div>
      </div>
      ...
      {/* Chat messages */}
      <div className="flex flex-col gap-2.5 overflow-hidden px-3 py-3">
      ...
      {/* Input bar */}
      <div className="absolute bottom-0 left-0 right-0 flex items-center gap-2 border-t border-stone-100 bg-white px-3 py-2.5">
      ...
      {/* Notch */}
      <div className="absolute left-1/2 top-[5px] h-4 w-20 -translate-x-1/2 rounded-full bg-stone-900/10" />
    </div>
  )
}
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          height: 520,
          background:
            "radial-gradient(circle, rgba(168,85,247,0.28) 0%, rgba(168,85,247,0.08) 55%, transparent 75%)",
          filter: "blur(80px)",
          borderRadius: "50%",
      ...
      >
        ...
        >
          {/* Badge */}
          <div className="mb-4 inline-flex items-center gap-1.5 rounded-full border border-purple-200 bg-purple-50 px-3.5 py-1 text-xs font-medium text-purple-700">
            <span className="h-1.5 w-1.5 rounded-full bg-purple-500 inline-block" />
            AI 가계부의 새로운 기준
          </div>

          {/* Heading with grape gradient */}
          <h1
            className="text-pretty text-4xl font-extrabold leading-tight sm:text-5xl md:text-[3.5rem]"
            style={{
              background:
                "linear-gradient(135deg, #7e22ce 0%, #a855f7 60%, #c084fc 100%)",
          ...
          >
            ...
            <span
              className="block"
              style={{ WebkitTextFillColor: "#292524", color: "#292524" }}
            >
              가계부
            </span>
          </h1>
