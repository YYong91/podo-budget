...
const members = [
  { name: "엄마", initials: "엄", color: "bg-[#f3e8ff] text-[#7e22ce]", border: "border-[#e9d5ff]" },
  { name: "아빠", initials: "아", color: "bg-[#dcfce7] text-[#15803d]", border: "border-[#bbf7d0]" },
  { name: "나",   initials: "나", color: "bg-[#fef9c3] text-[#a16207]", border: "border-[#fef08a]" },
]

const transactions = [
  { member: "엄마", color: "bg-[#f3e8ff] text-[#7e22ce]", text: "김치찌개 8,000원", time: "방금 전",   rotate: "-rotate-2", pos: "left-0 -top-6" },
  { member: "아빠", color: "bg-[#dcfce7] text-[#15803d]", text: "지하철 50,000원",  time: "3분 전",   rotate: "rotate-2",  pos: "right-0 top-4" },
      ...
      >
        ...
        >
          ...
          <div className="relative flex items-center">
            {members.map((m, i) => (
              <div
                key={i}
                className={`relative flex h-20 w-20 items-center justify-center rounded-full border-4 border-background text-2xl font-bold shadow-lg ${m.color} ${
                  i !== 0 ? "-ml-5" : ""
                }`}
              ...
              >
                {m.initials}
                {/* name chip */}
                <span className="absolute -bottom-7 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-card px-2.5 py-0.5 text-xs font-medium text-foreground shadow-sm border border-border">
                  {m.name}
                </span>
              </div>
            ))}
        ...
        >
          {["실시간 동기화", "멤버별 내역", "역할 구분", "초대 링크"].map((tag) => (
            <span
              key={tag}
              className="rounded-xl border border-border bg-card px-4 py-2 text-sm text-muted-foreground shadow-sm"
            >
              {tag}
            </span>
          ))}
