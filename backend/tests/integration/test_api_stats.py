...
async def create_test_expenses(db_session, user: User, household_id: int):
    ...
    expenses = [
        # 2026-01
        Expense(user_id=user.id, household_id=household_id, amount=10000, description="1월 식비1", category_id=cat_food.id, date=datetime(2026, 1, 5)),
        Expense(user_id=user.id, household_id=household_id, amount=5000, description="1월 교통1", category_id=cat_transport.id, date=datetime(2026, 1, 10)),
        Expense(user_id=user.id, household_id=household_id, amount=15000, description="1월 식비2", category_id=cat_food.id, date=datetime(2026, 1, 20)),
        # 2026-02
        Expense(user_id=user.id, household_id=household_id, amount=8000, description="2월 식비1", category_id=cat_food.id, date=datetime(2026, 2, 3)),
        Expense(user_id=user.id, household_id=household_id, amount=12000, description="2월 식비2", category_id=cat_food.id, date=datetime(2026, 2, 15)),
        Expense(user_id=user.id, household_id=household_id, amount=3000, description="2월 교통1", category_id=cat_transport.id, date=datetime(2026, 2, 15)),
        Expense(user_id=user.id, household_id=household_id, amount=7000, description="2월 식비3", category_id=cat_food.id, date=datetime(2026, 2, 20)),
        # 2026-03
        Expense(user_id=user.id, household_id=household_id, amount=20000, description="3월 식비1", category_id=cat_food.id, date=datetime(2026, 3, 1)),
        Expense(user_id=user.id, household_id=household_id, amount=6000, description="3월 교통1", category_id=cat_transport.id, date=datetime(2026, 3, 10)),
    ]
...
class TestStatsAPI:
    ...
    @pytest.mark.asyncio
    async def test_monthly_stats(self, authenticated_client, test_user, test_household, db_session):
        """월간 통계 조회"""
        await create_test_expenses(db_session, test_user, test_household.id)
    ...
    @pytest.mark.asyncio
    async def test_monthly_stats_by_category(self, authenticated_client, test_user, test_household, db_session):
        """월간 카테고리별 통계"""
        await create_test_expenses(db_session, test_user, test_household.id)
    ...
    @pytest.mark.asyncio
    async def test_monthly_stats_trend(self, authenticated_client, test_user, test_household, db_session):
        """월간 일별 트렌드"""
        await create_test_expenses(db_session, test_user, test_household.id)
    ...
    @pytest.mark.asyncio
    async def test_weekly_stats(self, authenticated_client, test_user, test_household, db_session):
        """주간 통계 조회"""
        await create_test_expenses(db_session, test_user, test_household.id)
    ...
    @pytest.mark.asyncio
    async def test_yearly_stats(self, authenticated_client, test_user, test_household, db_session):
        """연간 통계 조회"""
        await create_test_expenses(db_session, test_user, test_household.id)
    ...
    @pytest.mark.asyncio
    async def test_yearly_stats_trend_by_month(self, authenticated_client, test_user, test_household, db_session):
        """연간 월별 트렌드 값 확인"""
        await create_test_expenses(db_session, test_user, test_household.id)
...
class TestComparisonAPI:
    ...
    @pytest.mark.asyncio
    async def test_monthly_comparison(self, authenticated_client, test_user, test_household, db_session):
        """월간 비교 (전월 대비)"""
        await create_test_expenses(db_session, test_user, test_household.id)
    ...
    @pytest.mark.asyncio
    async def test_comparison_trend(self, authenticated_client, test_user, test_household, db_session):
        """3개월 트렌드"""
        await create_test_expenses(db_session, test_user, test_household.id)
    ...
    @pytest.mark.asyncio
    async def test_comparison_by_category(self, authenticated_client, test_user, test_household, db_session):
        """카테고리별 비교"""
        await create_test_expenses(db_session, test_user, test_household.id)
...
class TestExcludeFromStats:
    ...
    @pytest.mark.asyncio
    async def test_excluded_expense_not_in_monthly_stats(self, authenticated_client, test_user, test_household, db_session):
        """통계 제외 플래그가 설정된 지출은 월간 통계에서 제외된다."""
        normal = Expense(
            user_id=test_user.id, household_id=test_household.id, amount=10000, description="식비", date=datetime(2026, 2, 10), exclude_from_stats=False
        )
        excluded = Expense(
            user_id=test_user.id, household_id=test_household.id, amount=5000000, description="적금", date=datetime(2026, 2, 15), exclude_from_stats=True
        )
    ...
    @pytest.mark.asyncio
    async def test_excluded_expense_still_in_list(self, authenticated_client, test_user, test_household, db_session):
        """통계 제외 플래그가 설정된 지출도 목록에서는 정상 조회된다."""
        excluded = Expense(
            user_id=test_user.id, household_id=test_household.id, amount=5000000, description="퇴직금", date=datetime(2026, 2, 15), exclude_from_stats=True
        )
    ...
    @pytest.mark.asyncio
    async def test_excluded_expense_not_in_monthly_endpoint(self, authenticated_client, test_user, test_household, db_session):
        """GET /api/expenses/stats/monthly 에서도 통계 제외 거래가 빠진다."""
        normal = Expense(
            user_id=test_user.id, household_id=test_household.id, amount=8000, description="점심", date=datetime(2026, 2, 5), exclude_from_stats=False
        )
        excluded = Expense(
            user_id=test_user.id, household_id=test_household.id, amount=1000000, description="저축", date=datetime(2026, 2, 10), exclude_from_stats=True
        )
        db_session.add_all([normal, excluded])
        await db_session.commit()
